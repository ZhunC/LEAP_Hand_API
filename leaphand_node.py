...
class LeapNode:
    ...
    def __init__(self):
        ...
        self.curr_lim = float(
            rospy.get_param('/leaphand_node/curr_lim', 350.0)
        ...
        rospy.Subscriber(
            "/leaphand_node/cmd_leap", JointState, self._receive_pose
        )
        rospy.Subscriber(
            "/leaphand_node/cmd_allegro", JointState, self._receive_allegro
        )
        rospy.Subscriber(
            "/leaphand_node/cmd_ones", JointState, self._receive_ones
        )
        rospy.Subscriber("/joint_states", JointState, self._stretch_sense_set)
        ...
        self.dxl_client.sync_write(
            motors,
            np.ones(len(motors)) * self.kP, 84, 2
        )  # Pgain stiffness
        self.dxl_client.sync_write(
            [0, 4, 8],
            np.ones(3) * (self.kP * 0.75), 84, 2
        )  # Pgain stiffness for side to side should be a bit less
        self.dxl_client.sync_write(
            motors,
            np.ones(len(motors)) * self.kI, 82, 2
        )  # Igain
        self.dxl_client.sync_write(
            motors,
            np.ones(len(motors)) * self.kD, 80, 2
        )  # Dgain damping
        self.dxl_client.sync_write(
            [0, 4, 8],
            np.ones(3) * (self.kD * 0.75), 80, 2
        )  # Dgain damping for side to side should be a bit less
        #Max at current (in unit 1ma) so don't overheat and grip too hard #500 normal or #350 for lite
        self.dxl_client.sync_write(
            motors,
            np.ones(len(motors)) * self.curr_lim, 102, 2
        )
    ...
    #Receive LEAP pose by processing incoming values from /joint_states
    def _stretch_sense_set(self):
        print('Joint states received')
        input = pose.position  # [0, 4, 8, 12, 13 keep the same],
        pose = [
            0,
            input[3],
            input[3],
            input[3],  # index finger
            0,
            input[6],
            input[6],
            input[6],  # middle finger
            0,
            input[9],
            input[9],
            input[9],  # pinky finger
            0,
            0,
            input[1],
            input[1]  # thumb
